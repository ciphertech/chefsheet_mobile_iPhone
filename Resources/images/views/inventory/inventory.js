...
function setData() {
	...
	for ( i = 0; i < items[location_id].length; i++) {
		...
		var productitem = Titanium.UI.createLabel({
			text : p_name,
			font : {
				fontSize : (p_name.length >= 26) ? 10 : 14,
				fontWeight : 'bold'
			},
			width : '70%',
			textAlign : 'left',
		...
		if (product.quantity != null || (product.inventory_unit_qty != 0 && product.inventory_unit_qty != null) || (product.recipe_unit_qty != 0 && product.recipe_unit_qty != null)) {
			...
			if (product.quantity != null && unit  != 'No Unit' ) {
				...
				var productunit = Ti.UI.createLabel({
					text : product.inventory_unit_qty + ' ' + unit2,
					textAlign : 'right',
					font : {
						fontWeight : 'normal',
						fontSize : 14
			...
			} else if (product.recipe_unit_qty != 0 && product.recipe_unit_qty != null) {
				...
				var productunit = Ti.UI.createLabel({
					text : product.recipe_unit_qty + ' ' + unit3,
					textAlign : 'right',
					font : {
						fontWeight : 'normal',
						fontSize : 14
